...
public class ProfileSelect {
	...
	public void handle(KeyEvent e) {
		...
		case KeyEvent.VK_ENTER:
			if (curr_state == 2) {
				...
				curr_state = 1;
			} else if (curr_state == 1 && alpha < 100) {
				switch (curr_opt) {
				case 9: if (p.size() < 8) curr_state = 2; break;
				case 8: parent.set_profile("");	curr_state = 4;	break;
				default: parent.set_profile(p.get(curr_opt)); curr_state = 4; break;
				}
				break;
			}
		case KeyEvent.VK_Z:
	...
	public void note_pressed(byte id, byte vel, long timestamp) {
		...
		else if (c.contains("Amin")) { 
			if (curr_state == 2) curr_state = 1;
			else if (curr_state == 1) {
				curr_state = 3;
				bar_xc = proXL; bar_yc = proYB; bar_yc2 = proYT; alpha = 255; theta = (float)(Math.PI);
			}}
		else if (c.contains("Fmaj")) { 
			if (curr_state < 2) { 
				if (curr_opt == p.size() - 1) curr_opt = (short)8;	
				else curr_opt = (short)(curr_opt + 1 > max_opt? max_opt: curr_opt + 1);
			}} 
		else if (c.contains("Gmaj")) { 
			if (curr_state < 2) {
				if (curr_opt == 8) curr_opt = (short)(p.size() - 1);
				else curr_opt = (short)(curr_opt - 1 < 0 ? 0 : curr_opt - 1); 	
			}}
